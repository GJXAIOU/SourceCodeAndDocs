...
public class ThreadPoolExecutor extends AbstractExecutorService {
     ...
     *                  firstTask 是新创建的线程首先应该执行的任务（如果没有则为 null）。在方法 execute() 中 Workers
     *                  带着初始任务被创建，当总线程数小于核心线程数或者队列满了时候时候会绕过阻塞队列（即直接执行该任务），
     * @param core      if true use corePoolSize as bound, else
     *                  maximumPoolSize. (A boolean indicator is used here rather than a
     *                  value to ensure reads of fresh values after checking other pool
     *                  state).
     *                  core 为一个状态值，如果为 true 则用 corePoolSize 作为判断边界，如果为 false 则用 maximumPoolSize
     *                  作为判断边界
    ...
    /**
     * - corePoolSize：表示核心线程池的大小。当提交一个任务时，如果当前核心线程池的线程个数没有达到corePoolSize，则会创建新的线程来执行所提交的任务；反之不再创建线程。
     * - maximumPoolSize：表示线程池能创建线程的最大个数。如果阻塞队列已满，并且当前线程池的线程个数没有超过maximumPoolSize，就会创建新的线程执行任务。
     * <p>
     * - keepAliveTime：空闲线程存活时间。如果当前线程池的线程个数超过了corePoolSize，并且线程空闲时间超过了keepAliveTime
     * 的话，就会将这些空闲线程销毁，这样可以尽可能降低系统资源消耗。
     * <p>
     * - workQueue：阻塞队列。当核心线程已满时，将任务存储在队列中。
     * <p>
     * - threadFactory：创建线程的工厂类。
     * <p>
     * - handler：饱和策略。当线程池的阻塞队列已满，并且当前线程池的线程个数超过了maximumPoolSize，说明当前线程池已经处于饱和状态，那么就需要采用一种策略来处理这种情况；
     */
    ...
    /**
     * corePoolSize：线程池中维持的线程数量，即使它们处于空闲状态，除非设置了{@code allowCoreThreadTimeOut}
     *
     * @param maximumPoolSize   池中允许的最大线程数
     *
     * @param keepAliveTime     当线程数大于核心线程数目时，这是多余空闲线程在终止前等待新任务的最长时间。
     *
     * @param unit{@code keepAliveTime}        参数的时间单位
     *
     * @param workQueue        用于在执行任务之前保存任务的队列。这个队列将只保存由{@code execute}方法提交的{@code Runnable}任务。
     *
     * @param threadFactory    执行器创建新线程时要使用的工厂
     *
     * @param handler          由于达到线程边界和队列容量而阻止执行时要使用的处理程序
     *
     * @如果满足以下条件之一，则引发IllegalArgumentException:<br>
     *
     * {@code corePoolSize<0}<br>
     *
     * {@code keepAliveTime<0}<br>
     ...
     * {@code maximumPoolSize<corePoolSize}
     *
     * @如果{@code workQueue}或{@code threadFactory}或{@code handler}为nu，则引发NullPointerException
     */
    /**
     * Creates a new {@code ThreadPoolExecutor} with the given initial
     * parameters.
    ...
    /**
     * 第四种拒绝策略：如果被丢弃的线程任务未关闭，则执行该线程任务
     */
     ...
     */
    // 第三种拒绝策略：移除线程队列中最早的一个线程任务，并且尝试提交当前任务
